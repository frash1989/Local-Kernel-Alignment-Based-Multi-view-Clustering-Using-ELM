%% ELM iterWELM
function My_ELMC_LDA(data,numClust,truth,hidden)
...
for i=1:length(rho_set)
    ...
    for repeat =1:50
        [label,ite,cost,acc_tmp]=elmc_lda(data,paras);
        %acc(repeat)=accuracy(truth,label);
        
        [result,~] = ClusteringMeasure(truth, label);
        myacc(repeat) = result(1);
        mypurity(repeat) = result(3);
        [~,nmi(repeat),~] = compute_nmi (truth, label);
    end
